...
const Header = (props) => {
    ...
    const LightModeSwitch = styled(Switch)(({ theme }) => ({
        ...
        '& .MuiSwitch-thumb': {
            ...
            ...theme.applyStyles((lightMode ? 'light' : 'dark'), {
                backgroundColor: (lightMode ? '#efefef' : '#003892'),
            }),
        },
        '& .MuiSwitch-track': {
            opacity: 1,
    ...
    return (
        <header className={headerClasses.join(' ')}>
            <nav className='header-nav'>
                <div>
                    <Link to='/' className={lightMode ? 'header-light-link-title Large' : 'header-dark-link-title Large'}>MONATE</Link>
                </div>
